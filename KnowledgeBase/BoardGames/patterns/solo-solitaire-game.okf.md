...
Across the 16 games tagged `solo-solitaire-game`, only a minority carry reception evidence that actually discusses the dedicated solo mode as its own experience (as opposed to the mechanic simply being listed). Where reviewers do address solo play directly, the same tension recurs: a strong ruleset "just works" alone because it removes multiplayer downtime and negotiation overhead, but the solo variant itself is judged on whether it introduces its own new problems — swinginess from running a single hand, brain burn from multi-handing, an anticlimactic/procedural endgame, oversized physical footprint for one player, or (for a video-game adaptation) simply feeling like a slower manual reenactment of software logic. Onirim adds the strongest positive case yet: a reviewer names its solo mode "outstanding" and its short 10-15 minute sessions the game's clearest strength, while also noting the solo game becomes less challenging after repeated plays — the same "it just works, but only for a while" shape as other entries here. Evidence is genuinely thin for most co-op titles in this set; several carry the tag with no solo-specific reviewer commentary at all.
...
- onirim-second-edition (src-002): A reviewer with roughly twenty plays calls the solo mode outstanding, praising quick 10-15 minute sessions and ease of teaching, while also reporting the base solo game becomes "not very challenging" after repeated plays. confidence: high
...
- onirim-second-edition (src-002): quick 10-15 minute solo sessions and an easy teach are named as the game's outstanding strength, the strongest unqualified solo-mode praise found in this corpus.
