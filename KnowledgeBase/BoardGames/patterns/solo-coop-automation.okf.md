...
- horrified (src-002): the solo appendix changes the terror track and removes specific cards rather than supplying a separate automated opponent, making automation depth an improvement surface rather than a solved system. confidence: medium
- elder-sign (src-002, src-004): solo play works without any dedicated automation system, but reviewers say it leans on lenient death penalties — a solo player "could simply burn through several characters in a single game" — rather than designed solo-specific tension. confidence: high
- elder-sign (src-004): player-count balance is uneven in a way scaling rules could address, with the game reported "slightly easier with larger number of players" since damage and special powers spread across more characters. confidence: medium
...
- elder-sign (src-002, src-004): the absence of a dedicated solo-tension system is masked by lenient death penalties rather than addressed directly, and player-count balance drifts easier rather than staying flat.
- horrified (src-002): the solo appendix is a rules delta (terror change, card removal) grafted onto the multiplayer system rather than a purpose-built solo/automation layer.

## Coverage gaps

Dune Imperium's better-if doc proposes automating its House Hagal rival-priority logic for solo/two-player play, but the claim carries no direct source quote (only a general "evidence basis" note), so it couldn't be cited here — a case where the underlying source review would need re-checking for an actual quote before it strengthens this pattern. More broadly, none of the eight games in this sample use a card-driven "automa" bot deck (e.g., Scythe- or Wingspan-style); the corpus would benefit from a game with an explicit automaton/bot-opponent system to give this pattern genuine bot-AI-logic evidence rather than only solo-mode-scaling and co-op-cohesion complaints.
