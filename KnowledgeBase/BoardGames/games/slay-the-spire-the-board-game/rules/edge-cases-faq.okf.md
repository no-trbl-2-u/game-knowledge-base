...
- 2026-08-05 (librarian pass): Retried `src-002` with `curl -L` — still HTTP 200, still the same hosted copy (10.9 MB, 24 pages, "Sequential Turns" present, no "teardown" or "Ascension 7"/"A7" text anywhere in the extracted text); Contention Games has not updated this hosted URL to the v2.30 text described by src-003, so the open question is unchanged. `src-004` (Kickstarter FAQ) retried with `curl -L` — still HTTP 403. Neither retry advanced; no BGG filepage or Wayback re-check was attempted this pass beyond what 2026-07-08 already established.
